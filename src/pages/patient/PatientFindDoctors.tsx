...
const PatientFindDoctors = () => {
  useTitle("Find Doctors - Medical Universe");
  ...
  const fetchDoctors = async () => {
    try {
      ...
      // Fetch ONLY approved doctors from doctor_profiles table with availability info
      ...
      const transformedDoctors = (doctorProfiles || []).map(dp => {
        ...
        const doctorData = {
          ...
          // Add availability status fields - use correct column names
  ...
  const getAvailabilityDisplay = (doctor: any) => {
          ...
          <div className="bg-zinc-200 my-[70px] p-6 rounded-lg">
            ...
            {!isLoading && filteredDoctors.length === 0 && (
              <div className="text-center py-12">
                <p className="text-gray-500">
                  {doctors.length === 0 
                    ? "No approved doctors found. Please check back later." 
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-500">
                Total approved doctors: {doctors.length}
              </p>
            </div>
          </div>
        </main>
